...
if($payment_status == 'Completed'){
	...
	$result = ierg4210_order_fetchall();

	
	foreach($result as $r){
		if($r['TXNID'] == $txn_id) $flag = -1;
	}
	if($txn_type != "cart") $flag = -2;
	foreach($result as $r) {
		if($r['DIGEST'] == $record_id) {
			$salt = $r['SALT'];
			$productlist = $r['PRODUCTLIST'];
			$uuid = $r['UUID'];
			$totalprice = $r['TOTALPRICE'];
			$get_info=$r['PATMENTSTATUS'];
		}
	}
	$payment_amount=(int)$payment_amount;
	$info = array("merchantemail"=>$receiver_email, "salt"=>$salt, "currency"=>$payment_currency, "total_price"=>(string)$payment_amount);
	$items=json_decode($productlist);
	$return_array= array("info"=>$info, "items"=>$items);
	$digest =  hash_hmac('sha256', json_encode($return_array), $salt);
	if($digest != $record_id) $flag = -3;


	if($flag==1) {
		ierg4210_update_orders((int)$uuid, (string)$txn_id, (string)$payment_status); 
	}
}
